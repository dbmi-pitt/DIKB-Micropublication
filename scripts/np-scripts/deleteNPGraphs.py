...
def getNPGraphQry(name):
    qry = """
    
    SELECT DISTINCT ?s
WHERE { ?s a <http://www.nanopub.org/nschema#%s>.}
...
if __name__ == "__main__":
    ...
    #nanoPubQry = getNanoPubGraphQry("Nanopublication")
    #assertQry = getNPGraphQry("assertion")
    assertQry = getNPGraphQry("Provenance")
    ...
    for nanopub in nanopubL:
        graphname = nanopub.replace("http://purl.obolibrary.org/obo/","obo:") 
        print "[INFO] delete graph " + graphname
        isql.clean_graph(graphname)
        #isql.clean_graph("obo:DIDEO_XXX113-assertion")
